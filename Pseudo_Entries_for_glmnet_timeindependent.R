...
# Input: observed data
# Output: weighted pseduo entries for competing risk data with time-independent covariates to run in glmnet
FG.Surv=function(surv,cause, Z, id = 1:nrow(surv))
{
  ...
  if(min(C.surv) > 0)
  {
    C.knot = c(C.knot,max(C.Surv[,1])+1) # largest censoring time +1
  ...
  for(i in 1:length(id.unique))
  {
    ...
    {
      ck = C.knot[C.knot > X] # filter out the censoring times that are larger than the observation time X
      nk=length(ck)
      ...
      FG.id = c(FG.id, rep(id.unique[i],ni+nk-1)) # give id to pseudo entries
